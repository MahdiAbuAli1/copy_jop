...
# --- 3. الدوال الأساسية للنظام (تم إعادة بنائها بالكامل) ---

async def initialize_agent():
    """
    يقوم بتهيئة كل شيء من الصفر: تحميل المستندات، بناء الباحث الهجين، وتهيئة النموذج.
    """
    ...
    async with initialization_lock:
        ...
        # 2. تهيئة النموذج والـ Embeddings
        ...
        loader = DirectoryLoader(docs_path, glob="**/*.*", loader_cls=lambda p: PyPDFLoader(p) if p.endswith('.pdf') else TextLoader(p, encoding='utf-8'), show_progress=True, use_multithreading=True)
        docs = loader.load()
        ...
        # 4. بناء الباحث الهجين (Ensemble Retriever)
        logging.info("جارٍ بناء الباحث الهجين...")
        
        # الباحث الأول: البحث بالكلمات المفتاحية (BM25)
        bm25_retriever = BM25Retriever.from_documents(splits)
        bm25_retriever.k = 5 # اجلب أفضل 5 نتائج مطابقة للكلمات

        # الباحث الثاني: البحث بالتشابه الدلالي (FAISS)
...
# --- 4. دالة get_answer_stream - النسخة النهائية ---
async def get_answer_stream(request_info: dict) -> AsyncGenerator[Dict, None]:
    ...
    def format_docs(docs: List) -> str:
        # إزالة التكرار من المستندات المدمجة
        unique_contents = set()
        unique_docs = []
        for doc in docs:
